...
module Target: sig
  ...
  type 'a t = [`target of Filename.t | `targets of Filename.t list [@sexp.list]] [@@deriving_inline sexp_of]
...
let sexp_of_t : 'a . ('a -> Sexplib0.Sexp.t) -> 'a t -> Sexplib0.Sexp.t =
  fun _of_a__001_ ->
    ...
    | `targets l__003_ ->
        Sexplib0.Sexp.List ((Sexplib0.Sexp.Atom "targets") ::
          (Sexplib0.Sexp_conv.list_map Filename.sexp_of_t l__003_))
let _ = sexp_of_t
[@@@end]

  let split_base : fullpath t -> Fpath.t * basepath t = function
    | `target p -> let base, p = Fpath.split_base p in base, `target p
    | `targets lst ->
      let targets =
        lst |> List.to_seq |> Seq.map Fpath.split_base |> Filename.Map.of_seq in
      if Filename.Map.cardinal targets = 1 then
        let base, p = Filename.Map.choose targets in base, `target p
      else
        Fmt.invalid_arg "No common base path for targets: %a"
          Fmt.(Dump.iter_bindings Filename.Map.iter (any "map") Fpath.pp Fpath.pp) targets

  let compare a b = match a, b with
    |`target f1, `target f2 -> Fpath.compare f1 f2
    | `targets l1, `targets l2 -> List.compare Fpath.compare l1 l2
    | `target _, `targets _ -> -1
    | `targets _, `target _ -> 1
...
module Dep = struct
  type sandbox = [`always | `none | `preserve_file_kind] [@@deriving_inline sexp_of]
  
let _ = fun (_ : sandbox) -> ()
let sexp_of_sandbox =
  (function
   | `always -> Sexplib0.Sexp.Atom "always"
   | `none -> Sexplib0.Sexp.Atom "none"
   | `preserve_file_kind -> Sexplib0.Sexp.Atom "preserve_file_kind" :
...
let sexp_of_t =
  (function
   ...
   | #Include.t as v__012_ -> Include.sexp_of_t v__012_ : t ->
                                                            Sexplib0.Sexp.t)
let _ = sexp_of_t
[@@@end]
end
...
let sexp_of_t =
  (fun (`deps l__013_) ->
     Sexplib0.Sexp.List ((Sexplib0.Sexp.Atom "deps") ::
       (Sexplib0.Sexp_conv.list_map Dep.sexp_of_t l__013_)) : t ->
                                                                Sexplib0.Sexp.t)
let _ = sexp_of_t
[@@@end]
end
...
let sexp_of_t =
  (function
   | `standard -> Sexplib0.Sexp.Atom "standard"
   | `fallback -> Sexplib0.Sexp.Atom "fallback"
   | `promote -> Sexplib0.Sexp.Atom "promote" : t -> Sexplib0.Sexp.t)
let _ = sexp_of_t
[@@@end]
end

module Rule = struct
  type 'a t = [`rule] * 'a Target.t * Deps.t * Action.t * Mode.t option [@sexp.option] [@@deriving_inline sexp_of]
  
let _ = fun (_ : 'a t) -> ()
let sexp_of_t : 'a . ('a -> Sexplib0.Sexp.t) -> 'a t -> Sexplib0.Sexp.t =
  fun _of_a__014_ ->
    fun (arg0__015_, arg1__016_, arg2__017_, arg3__018_, arg4__019_) ->
      let res0__020_ = let `rule = arg0__015_ in Sexplib0.Sexp.Atom "rule"
...
let rule ?mode target deps action : t =
   ...
   Filename.Map.singleton base @@ TargetMap.singleton target (`rule, target, deps, action, mode)

let merge : t -> t -> t = Filename.Map.union @@ fun _ t1 t2 ->
  Some (TargetMap.union (fun target r1 r2 ->
...
module Subdir = struct
  type t = [`subdir] * Filename.t * Target.basepath Rule.t list [@@deriving_inline sexp_of]
  
let _ = fun (_ : t) -> ()
let sexp_of_t =
  (fun (arg0__025_, arg1__026_, arg2__027_) ->
     let res0__028_ = let `subdir = arg0__025_ in Sexplib0.Sexp.Atom "subdir"
     and res1__029_ = Filename.sexp_of_t arg1__026_
     and res2__030_ =
       sexp_of_list (Rule.sexp_of_t Target.sexp_of_basepath) arg2__027_ in
     Sexplib0.Sexp.List [res0__028_; res1__029_; res2__030_] : t ->
                                                                 Sexplib0.Sexp.t)
let _ = sexp_of_t
[@@@end]
end

let subdir base rules : Subdir.t =
  `subdir, base, rules

let sexp_of_t t =
  let rules =
    t |> Filename.Map.to_seq |> Seq.flat_map @@ fun (base, targets) ->
    let target_rules : Target.basepath Rule.t Seq.t = targets |> TargetMap.to_seq |> Seq.map snd in
